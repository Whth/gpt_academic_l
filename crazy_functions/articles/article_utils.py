...
并且，一定要注意在正文中引用时要带上前三个作者的姓与年份，这里假设作者1，2，3是虚拟的的作者，正常情况下你需要使用实际对应的作者名来替换它们,一般如果一篇论文有作者三个以上，你可以说“作者1，作者2，作者3等人（2013）做了什么什么,...”。如果作者只有两个，你可以说“作者1与作者2（2013）做了什么什么,...”。如果只有一个人，你可以说“作者1（2013）做了什么什么,...”。
同样的，如果你需要同时引用多篇论文，那么你就要在年份后面加入多个占位标签字段，比如“作者1（2019），作者2（2024）做了什么什么，...”。
额外的，你可以参考参考文献中的图表，如果图表中存在可以迁移到我的文章中的图表来作为对于特定内容的辅助说明，你可以在正文中引用这些图表，不要忘了在引用后面加入占位标签字段。引文直接在正文中使用图或表的名称，比如你要引用作者1，作者2，作者3等人（2016）的论文中的“图-叶片疲劳曲线”，你可以说“作者1，作者2，作者3等人（2016）的图-叶片疲劳曲线所示，表明了什么什么，印证了什么什么，...”。最后我会帮你完成多媒体文件的插入，所以你只需要用字符说明哪里要引用
章节编号一定要按照我的提纲的来，不要自己随意增加或者减少章节。最后的你给出结果的末尾你也不用添加参考文献的尾注，我会自行添加。
也不要使用任何的Markdown语法，输出内容应该是Plaintext，你应该严格按照x x.y x.y.z这样的标题序号规范排版。      
...
"""
    ...
    def _write_iter_asm_inner(self,grouped:List[Path],written_article_place_holder:str)->str:
        ...
        return (
            ...
            "并且，一定要注意在正文中引用时要带上前三个作者的姓与年份，这里假设作者1，2，3是虚拟的的作者，正常情况下你需要使用实际对应的作者名来替换它们,"
            "一般如果一篇论文有作者三个以上，你可以说“作者1，作者2，作者3等人（2013）做了什么什么,...”。"
            "如果作者只有两个，你可以说“作者1与作者2（2013）做了什么什么,...”。如果只有一个人，你可以说“作者1（2013）做了什么什么,...”。\n"
            "同样的，如果你需要同时引用多篇论文，那么你就要在年份后面加入多个占位标签字段，比如“作者1（2019），作者2（2024）做了什么什么，...”。\n"
            "额外的，你可以参考参考文献中的图表，如果图表中存在可以迁移到我的文章中的图表来作为对于特定内容的辅助说明，"
            "你可以在正文中引用这些图表，不要忘了在引用后面加入占位标签字段。引文直接在正文中使用图或表的名称，比如你要引用作者1，作者2，作者3等人（2016）的论文中的“图-叶片疲劳曲线”，"
            "你可以说“如作者1，作者2，作者3等人（2016）的图-叶片疲劳曲线所示，表明了什么什么，印证了什么什么，...”。最后我会帮你完成多媒体文件的插入，所以你只需要用字符说明哪里要引用\n"
            "额外的，你不用在开始写的时候表示“好的”，也不用在写完了之后表示“完成了”,直接给出结果就可以。\n"
            "除了答案外不要有额外的说明！也不要使用任何的Markdown语法，输出内容应该是Plaintext，你应该严格按照x x.y x.y.z这样的标题序号规范排版。"      
    ...
    def get_incited(refs: List[Path], response:str) -> List[Path]:
        ...
        cited_incorrectly=filter(lambda x:x.year not in response or # 作者年份不在回答中
                          abs(index(x.authors.split(" ")[0])-index(x.year))>60, # 作者年份在回答中，但是位置不对
                 cited_segs)
